...
def crypto():
    ...
    klines = client.get_historical_klines(symbol, interval, "22 Jul, 2022") #alterar data aqui
    ...
    try:
        ...
        isoch_final = np.concatenate((isoch_0,isoch_1, isoch_2), axis = 0)
        ...
        plt.plot(xplot0,isoch_final, label='isocrona (previsão)',alpha=0.6, marker='.')
        # plt.scatter(xplot0,isoch_final)
        plt.plot(xplot1,target, label='target',alpha=0.6, marker='.')
